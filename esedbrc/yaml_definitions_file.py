...
class YAMLDatabaseDefinitionsFile:
    ...
    * database_identifier, identifier of the database type.
    ...
    _SUPPORTED_KEYS = frozenset(["artifact_definition", "database_identifier"])
    ...
    def _ReadDatabaseDefinition(self, yaml_database_definition):
        ...
        artifact_definition = yaml_database_definition.get("artifact_definition", None)
        if not artifact_definition:
            raise RuntimeError("Invalid database definition missing format identifier.")

        database_identifier = yaml_database_definition.get("database_identifier", None)
